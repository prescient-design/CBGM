...
def generate_data(
    ...
    if use_binary_vector_class:
        ...
        if config.get("num_hidden_concepts", 0):
            ...
            hidden_concepts = sorted(
                sorted_concepts[
                    num_concepts : min(
                        (num_concepts + num_hidden), len(sorted_concepts)
                    )
                ]
            )
        else:
        ...
        selected_concepts_ = [CONCEPT_SEMANTICS[i] for i in concept_idxs]
    else:
        ...
        sorted_labels = list(
            map(lambda x: x[0], sorted(zip(vals, counts), key=lambda x: -x[1]))
        )
    ...
    if not ds_for_generation:
        ...
        if not output_dataset_vars:
            return train_dl, val_dl, test_dl, selected_concepts_
