...
def reparameterization(mu, logvar, latent_dim):
    ...
    sampled_z = Variable(
        torch.Tensor(np.random.normal(0, 1, (mu.size(0), latent_dim)))
    ).cuda()
    z = sampled_z * std + mu
    return z
...
class Generator_Simple(nn.Module):
    def __init__(self, latent_dim: int, num_channels: int):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.ConvTranspose2d(
                self.h2_nchan, self.h3_nchan, kernel_size=4, stride=2, padding=1
            ),
        ...
        self.conv2 = nn.Sequential(
            ...
            #           kernel_size=5, stride=1, padding=2),
            nn.ConvTranspose2d(
                self.h3_nchan, num_channels, kernel_size=4, stride=2, padding=1
            ),
            nn.Sigmoid(),
        )
