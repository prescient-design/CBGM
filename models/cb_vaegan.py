...
class CBM_plus_Dec(nn.Module):
        ...
        for c in range(self.n_concepts):
            ...
            self.concept_context_generators.append(
                torch.nn.Sequential(
                    *[
                        torch.nn.Linear(
                            self.noise_dim, self.concept_bins[c] * self.emb_size
                        ),
    ...
    def forward(self, h, probs=None, return_all=False):
        ...
        latent = (
            torch.cat((all_concepts, all_concept_latent, non_concept_latent), 1)
            .unsqueeze(-1)
            .unsqueeze(-1)
        )
        ...
        for c in range(self.n_concepts):
            if self.concept_type[c] == "cat":
                ...
                cat_onehot = torch.zeros(
                    num, self.concept_bins[c], dtype=torch.float, device=self.device
                )
