    ...
    for epoch in range(config["train_config"]["epochs"]):
        ...
        for i, (imgs, concepts) in enumerate(dataloader):
            ...
            (
                _,
                logits_given_real,
                concept_context_given_real,
                nonconcept_context_given_real,
            ) = model.dec(latent, return_all=True)

            latent = model.sample_latent(batch_size)
            (
                _,
                logits_given_latent,
                concept_context_given_latent,
                nonconcept_context_given_latent,
            ) = model.dec(latent, return_all=True)

            concept_loss, concept_loss_lst = get_concept_loss(
                model, logits_given_real, real_concepts
            )

            orthognality_loss = 0
            for c in range(model.n_concepts):
                orthognality_loss += gan_loss.OrthogonalProjectionLoss(
                    concept_context_given_real[
                        :, c * model.emb_size : (c * model.emb_size) + model.emb_size
                    ],
                    nonconcept_context_given_real,
                )
                orthognality_loss += gan_loss.OrthogonalProjectionLoss(
